...
fn calculate_mandelbrot(
    // adding extra line spaces for long annotations
    max_iters: usize,
    x_min: f64,
    x_max: f64,
    y_min: f64,
    ...
    // add a line break to the following (55 max char limit)
    let mut all_rows: Vec<Vec<usize>> = Vec::with_capacity(width);
    for img_y in 0..height {
        let mut row: Vec<usize> = Vec::with_capacity(height);
        for img_x in 0..width {
            // add a line break to the following three lines (55 max char limit)
